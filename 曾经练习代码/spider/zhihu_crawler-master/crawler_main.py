...
class ZhihuCrawler(object):
    ...
    def _load_config(self):
        struct = {"account": r"zilanxingshuo@gmail.com", "password": r"mzczlxs",
    ...
    def _save_user(self, user):
        with open(ZhihuCommon.user_json_file, "a", encoding="utf-8") as fp:
            json_str = json.dumps(
                user, default=ZhihuUser.obj_to_dict, ensure_ascii=False, sort_keys=True)
            fp.write(json_str + "\n")

    def _save_answer(self, answer):
        with open(ZhihuCommon.answer_json_file, "a", encoding="utf-8") as fp:
            json_str = json.dumps(
                answer, default=ZhihuAnswer.obj_to_dict, ensure_ascii=False, sort_keys=True)
            fp.write(json_str + "\n")

    def _save_topic(self, topic):
        with open(ZhihuCommon.topic_json_file, "a", encoding="utf-8") as fp:
            json_str = json.dumps(
                topic, default=ZhihuTopic.obj_to_dict, ensure_ascii=False, sort_keys=True)
            fp.write(json_str + "\n")
...
class ZhihuTopic(object):
    ...
    def __init__(self, topic_id):
        ...
        if self._valid:
            ...
            self._debug_print(
                DebugLevel.verbose, "find " + str(len(self._top_answer_urls)) + " answers")
            self._debug_print(
                DebugLevel.verbose, "parse " + url + ". topic " + self._name + " OK!")
    ...
    def _parse_child_topic(self):
        ...
        topic_tree_url = r"https://www.zhihu.com/topic/" + \
            str(self._topic_id) + r"/organize/entire"
        post_dict = {
            '_xsrf': ZhihuCommon.get_xsrf()
        }
        ...
        while continue_load:
            if first_query:
                ...
                query_url = topic_tree_url + r"?child=" + \
                    last_topic + r"&parent=" + parent_topic
    ...
    def _parse_top_answer_one_page(self, page):
        ...
        try:
            ...
            self._debug_print(
                DebugLevel.warning, "fail to get page " + page_url + "errinfo " + str(e))
...
class ZhihuAnswer(object):
    ...
    def _parse_answer(self):
        ...
        try:
            ...
            time.sleep(10)
            self._debug_print(
                DebugLevel.warning, "fail to parse " + self._url + "ErrInfo: " + str(e))
...
class ZhihuUser(object):
    ...
    def _parse_user_page(self):
        try:
            ...
            agree_tag = head_tag.find(
                "span", class_="zm-profile-header-user-agree")
            agree_cnt = agree_tag.contents[1].contents[0]
            thank_tag = head_tag.find(
                "span", class_="zm-profile-header-user-thanks")
            ...
            is_ok = True
            self._debug_print(
                DebugLevel.verbose, "parse " + self._user_url + " ok. " + "name:" + self._name)
        except Exception as e:
...
class ZhihuCommon(object):
    ...
    def get(url):
        ...
        while try_time < 5:
            ...
            try:
                try_time += 1
                response = ZhihuCommon.get_session().get(
                    url, headers=ZhihuCommon.my_header, timeout=30)
                #, cert = 'F:\Programs\Class-3-Public-Primary-Certification-Authority.pem')
                soup = BeautifulSoup(response.text, "html.parser")
                ZhihuCommon._last_get_page_fail = False
                return response.text, soup
            except Exception as e:
                print("fail to get " + url + " error info: " +
                      str(e) + " try_time " + str(try_time))
    ...
    def post(url, post_dict):
        ...
        while try_time < 5:
            ...
            try:
                try_time += 1
                response = ZhihuCommon.get_session().post(
                    url, headers=ZhihuCommon.my_header, data=post_dict, timeout=30)
                #, cert = 'F:\Programs\Class-3-Public-Primary-Certification-Authority.pem')
                ZhihuCommon._last_get_page_fail = False
                return response
            except Exception as e:
                print("fail to post " + url + " error info: " +
                      str(e) + " try_time " + str(try_time))
                ZhihuCommon._last_get_page_fail = True
        else:
            raise  # 当前函数不知道应该怎么处理该错误，所以，最恰当的方式是继续往上抛，让顶层调用者去处理
    ...
    def get_and_save_page(url, path):
        try:
            response = ZhihuCommon.get_session().get(
                url, headers=ZhihuCommon.my_header,  verify=False)
...
class ZhihuAnalyse(object):
    ...
    def _analyse_user(self):
        ...
        sorted_user_edu = sorted(
            self.user_edu.items(), key=lambda ele: ele[1], reverse=True)
        print("edu info: " + str(sorted_user_edu))
        # 对self.user_edu_major按数量排序
        sorted_user_major = sorted(
            self.user_edu_major.items(), key=lambda ele: ele[1], reverse=True)
        print("edu major: " + str(sorted_user_major))
        # 对self.user_employ按数量排序
        sorted_user_employ = sorted(
            self.user_employ.items(), key=lambda ele: ele[1], reverse=True)
